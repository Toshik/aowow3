...
var fi_filters = {
	...
	{
		id: 118,
		name: "purchasablewith",
		type: "currency-any"
	},
	{
		id: 145,
		name: "purchasablewitharena",
		type: "yn"
	},
	{
		id: 144,
		name: "purchasablewithhonor",
		type: "yn"
	},
	{
		id: 18,
	...
	for (var b = 0; b < a.length; ++b) {
		...
		case "INPUT":
			...
			case "checkbox":
				...
				break;
			case "radio":
				if (a[b].name == "gb" && a[b].checked && a[b].value) {
					++c
				}
				break
			}
	...
	if (fi_upgradeId) {
		for (var a = 0; a < this.data.length; ++a) {
			...
			if (b.id == fi_upgradeId) {
				this.createIndicator(sprintf(LANG.lvnote_upgradesfor, b.id, (7 - parseInt(b.name.charAt(0))), b.name.substr(1)), location.href.replace(/;upg=[0-9]+/i, ""));
				break
			}
		}
	}
...
function fi_init(a) {
	fi_type = a;
	fi_initCriterion(ge("fi_criteria"), "cr[]", a);
	if (a == "items") {
		var c = ge("fi_presets");
		if (c) {
			fi_initPresets(ge("fi_presets"));
			fi_initCriterion(ge("fi_weight"), "wt[]", a)
		}
	}
	var b = ge("ma-0");
...
function fi_initPresets(h) {
	...
	k.href = "javascript:;";
	k.id = "fi_detail";
	k.appendChild(ct(LANG.fishowdetails));
	k.onclick = fi_presetDetails;
	k.onmousedown = rf;
	ae(h, k);
	var b = ge("gnjndfgkjdfg35");
	if (b) {
		g_addTooltip(b, LANG.tooltip_statweighting, "q")
	}
}
...
function fi_mergeFilterParams(c, e) {
	var a = fi_filterParamToJson(c);
	var b = fi_filterParamToJson(e);
	var d = $.extend(b, a);
	return fi_filterJsonToParam(d)
}
function fi_filterParamToJson(a) {
	var b = {};
	var c = a.split(";");
	$.each(c, function (d, e) {
		g_splitQueryParam(e, b)
	});
	return b
}
